...
Additionally, I quickly looked at the uncertainty around the three parameters in the model (the intercept, and the coefficients for the two predictors). See plot below, where the point represents the central estimate for the parameter, and the thick dark blue line represents a 50% uncertainty interval and the thin blue line a 90% uncertainty interval. Reviewing the plot I noted that:

-   There is very little uncertainty associated with the co-efficents of the two predictor variables.

-   So, the uncertainty around the model parameter is effectively restricted to the value of the intercept of the regression line.

```{r}
# process posterior for interval plots
posterior <- as.array(lm_3)

# plot uncertainty around model parameters
mcmc_intervals(posterior, pars = c("(Intercept)", "log_gdp", 
                                   "log_population"))
```
...
median_preds <- preds %>% 
  ...
  pivot_longer(cols = everything(), values_to = "median_pred")

# sample n predictions from posterior predictive distribution
draws <- preds %>% 
  t() %>% 
  as_tibble()

sampled_preds <- draws[, sample(ncol(draws), 50)]

# process predictions for plotting
waste_by_year_country <- pred_plastic_waste %>% 
  bind_cols(median_preds, sampled_preds) %>%
  group_by(year, country) %>% 
  summarise(across(median_pred:last_col(), sum)) 

waste_by_year_country_med <- waste_by_year_country %>% 
  select(-starts_with("V"))

waste_by_year_country_draws <- waste_by_year_country %>% 
  select(-median_pred) %>%
  pivot_longer(starts_with("V"), names_to = "variable", values_to = "value") 

# produce plot
ggplot(waste_by_year_country_med) +
  
  geom_line(aes(year, exp(median_pred)), 
            size = 3, colour = "red"
           ) +
  
  geom_line(data = waste_by_year_country_draws,
            mapping = aes(year, exp(value), group = variable), alpha = 0.3) +
  
  scale_x_continuous(breaks = c(2011, 2012, 2013)) +
  scale_y_continuous(labels = scales::label_number_si(accuracy=0.1)) +
  
  labs(x = NULL,
       y = "Predicted annual plastic waste production\n") +
